...
Remix has a built-in API for routes defining the `meta` tags that should be used
when the route is active through
...
function so you can merge them yourself if you so desire (you do).
...
Please recall that in Remix you're responsible for everything on the page
between `<html>` and `</html>` and that's no different here. You need to apply
the [`<Meta />`](https://remix.run/docs/en/main/components/meta) component in
the `<head>` of your document in your `app/root.tsx` to ensure these meta tags
are rendered.
...
export const meta: V2_MetaFunction<typeof loader> = ({ loaderData }) => {
	return [
