...
and you'll be using some utilities from <InlineFile file="app/utils/auth.server.ts" />.
Luckily, you shouldn't need to make any changes to that file, but some of those
functions will help you determine who the current user is.
...
password. You'll need this when you test out the <LinkToApp to="/settings/profile" /> page.
(🐨 Kody insisted on being an exception to this rule. His username is `kody` and
his password is `kodylovesyou` 🙄 🥰).
